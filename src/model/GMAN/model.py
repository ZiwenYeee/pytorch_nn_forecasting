...
class STEmbedding(nn.Module):
    ...
    def __init__(self, slot_num, D, bn_decay):
        ...
        self.FC_te = FC(
            input_dims=[2 * D, D], units=[D, D], activations=[F.relu, None],
            bn_decay=bn_decay)  # input_dims = time step per day + days per week=288+7=295
        
        self.weekday_embedding = nn.Embedding(7, D)
        self.timeslot_embedding = nn.Embedding(slot_num, D)

    def forward(self, SE, past_weekday, future_weekday, past_time_slot, future_time_slot, T=288):
        # spatial embedding
        ...
        #past_weekday [B, T]
        weekday = torch.cat([past_weekday, future_weekday], dim = 1)
        timeslot = torch.cat([past_time_slot, future_time_slot], dim = 1)
        
        dayofweek = self.weekday_embedding(weekday)
        timeofday = self.timeslot_embedding(timeslot)
...
class GMAN(nn.Module):
    ...
    GMAN
        ...
        block_num：       one day is divided into T steps
        L：       number of STAtt blocks in the encoder/decoder
        n_head：       number of attention heads
        n_dim：       dimension of each attention head outputs
    ...
    def __init__(self, 
        ...
        self.d = d = n_dim
        self.D = D = K * d
        self.SE = nn.Parameter(torch.randn(num_vertex, D), requires_grad=True)
        #self.SE = SE
        self.STEmbedding = STEmbedding(slot_num, D, bn_decay)
    ...
    def forward(self, past_target, 
                ...
                past_time_slot, future_time_slot):
        num_his = past_target.shape[1]

        # input
        X = torch.unsqueeze(past_target, -1)
        X = self.FC_1(X)
        # STE
        STE = self.STEmbedding(self.SE, past_weekday, future_weekday,
                                past_time_slot, future_time_slot)
